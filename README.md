# 릴스 대본 기획 도구 (claude-reels-planner)
...
│   └── skills/릴스대본기획/       도구 본체 (업데이트 때 이 폴더만 교체)
│       ├── SKILL.md               절차
│       ├── patterns/              점검표 3종 (기획 5단계 · AI 티 검사 · 출력 형식)
│       └── lib/fetch.sh           대시보드 데이터 조회
├── 계정/
...
| 되는 것 | 준비 중 (v0.2) |
|---|---|
| 스킬 본체 · 점검표 · 계정 양식 · 구축 가이드 | `fetch.sh`의 크리에이터 대시보드(JSON) 읽기 — 지금은 Supabase형 대시보드만 자동, JSON은 Claude가 파일을 직접 읽음 |
| 수동 모드(대본 붙여 넣기)로 전 과정 | `/계정세팅` 스킬 — 지금은 SETUP.md 3단계대로 Claude가 직접 초안 작성 |

## 함께 쓰는 도구

- 대본추출 — 릴스 주소에서 음성을 글로: https://github.com/gfmmmm/claude-video-transcript
- 크리에이터 대시보드 — 본인 릴스·레퍼런스 수집·분석 (준비 중)
